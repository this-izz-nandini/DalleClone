...
const CreatePost = () => {
    ...
    const generateImage = async () => {
        if (form.prompt) {
            try {
                ...
                const response = await fetch('https://dalleclone-z8i5.onrender.com/api/v1/dalle', {
                    method: 'POST',
    ...
    const handleSubmit = async (e) => {
        ...
        if (form.prompt && form.photo) {
            setLoading(true);
            try {
                const respone = await fetch('https://dalleclone-z8i5.onrender.com/api/v1/post', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', },
            ...
            <form className='mt-16 max-w-3xl' onSubmit={handleSubmit}>
                <div className='flex flex-col gap-5'>
                    <FormField labelName='Your name' type='text' name='name'
                        placeholder='Peter Pan' value={form.name} handleChange={handleChange}
                    />
                    <FormField labelName='Prompt' type='text' name='prompt'
                        placeholder='a painting of a fox in the style of Starry Night'
                        value={form.prompt} handleChange={handleChange} isSurpriseMe
                        handleSurpriseMe={handleSurpriseMe}
                    />
                    <div className='relative bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 h-64 w-64 p-3 flex justify-center items-center'>
